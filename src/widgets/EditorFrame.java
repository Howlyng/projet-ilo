...
public class EditorFrame extends JFrame implements Signature
{
	...
	public EditorFrame() throws HeadlessException
	{
		...
		edgeColorComboBox.setPreferredSize(new Dimension(80, 32));
		edgeColorComboBox.setAlignmentX(0.5f);
		...
		fillColorComboBox.setPreferredSize(new Dimension(80, 32));
		fillColorComboBox.setAlignmentX(0.5f);
		...
		lineTypeComboBox.setPreferredSize(new Dimension(80, 32));
		...
		fillColorComboBox.addItemListener(new ColorItemListener(fillPaints, defaultFillColorIndex, specialFillColorIndex, PaintToType.FILL));
		edgeColorComboBox.addItemListener(new ColorItemListener(edgePaints, defaultEdgeColorIndex, specialEdgeColorIndex, PaintToType.EDGE));
		lineTypeComboBox.addItemListener(new EdgeTypeListener(LineType.fromInteger(lineTypeComboBox.getSelectedIndex())));
	...
	private class ToggleCreateEditAction extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent event)
		{
			...
			for (Iterator<AbstractButton> b = buttons.iterator(); b.hasNext();) {
				AbstractButton suivant = b.next();
				if (suivant.isSelected() != selected) {
					suivant.setSelected(selected);
				}
			}
	...
	private class LineFilterAction extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent event)
		{
			AbstractButton button = (AbstractButton) event.getSource();
			boolean selected = button.getModel().isSelected();
			LineFilter line_f = new LineFilter(type);
			if (selected){
				drawingModel.addLineFilter(line_f);
			}
			else{
				drawingModel.removeLineFilter(line_f);
			}
		}
	}
		...
		public void itemStateChanged(ItemEvent e)
		{
			...
			switch (stateChange)
			{
				case ItemEvent.SELECTED:
				{
					...
					AbstractCreationListener abstractCreationListener = figureType.getCreationListener(drawingModel, history,infoLabel);
					if(operationMode == OperationMode.CREATION){
						drawingPanel.removeFigureListener(creationListener);
						drawingPanel.addFigureListener(abstractCreationListener);
					}

					creationListener=abstractCreationListener;
					break;
				}
			}
		}
		...
		public void itemStateChanged(ItemEvent e)
		{
			...
			if ((index >= 0) && (index < colors.length))
			{
				if (e.getStateChange() == ItemEvent.SELECTED)
				{
					...
					{
						/*
						 * Utiliser PaintFactory.getPaint(combo, ...)
						 * pour obtenir une boite de diaglogue permettant
						 * de choisir la couleur dans une palette
						 */
						Paint chosenColor = PaintFactory.getPaint(combo, "Choose " + applyTo.toString() + " Color", lastColor);

						/*
						 * Si chosenColor est valide alors la copier
					...
					{
						// copier la coulors de index dans #paint
						paint = colors[index];
					}
				...
				else if (e.getStateChange() == ItemEvent.DESELECTED)
				{
					...
					{
						/*
						 * stocker la couleur de cet index dans #lastColor
	...
	private class EdgeTypeListener implements ItemListener
	{
		...
		public void itemStateChanged(ItemEvent e)
		{
			...
			if (e.getStateChange() == ItemEvent.SELECTED)
			{
				...
				/*
				 * Obtenir le LineType à partir de l'index,
	...
	private class EdgeWidthListener implements ChangeListener
	{
		...
		public void stateChanged(ChangeEvent e)
		{
			...
			/*
			 * obtenir le nombre du spinnerModel puis mettre en place
